...
public class CpmController extends GUIController implements Initializable {
    @FXML
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        try {
            ...
            createGranularityChart(controller.computeCPM(filter, Controller.GRAN_DAY), filter, Controller.GRAN_DAY, lineChart);
    ...
    public double setStat(Date start, Date end, int granularity) throws SQLException {
        float[] data = controller.computeCPM(new Filter(start, end), granularity);
        return data[0];
    }
    ...
    public void showGraph() throws ParseException, SQLException {
        ...
        try {
            switch (granularityDrop.getValue()) {
                case "Day granularity" -> createGranularityChart(controller.computeCPM(filter, Controller.GRAN_DAY), filter, Controller.GRAN_DAY, lineChart);
                case "Week granularity" -> createGranularityChart(controller.computeCPM(filter, Controller.GRAN_WEEK), filter, Controller.GRAN_WEEK, lineChart);
                case "Month granularity" -> createGranularityChart(controller.computeCPM(filter, Controller.GRAN_MONTH), filter, Controller.GRAN_MONTH, lineChart);
                case "Time of day" -> createPerformanceMetricChart(controller.computeCPMPM(filter, Controller.PERFORMANCE_DAY), filter, Controller.PERFORMANCE_DAY, lineChart);
                case "Day of week" -> createPerformanceMetricChart(controller.computeCPMPM(filter, Controller.PERFORMANCE_WEEK), filter, Controller.PERFORMANCE_WEEK, lineChart);
...
//        float[] days = controller.computeCPMByGranularity(start, end, interval);
...
//
//        float[] ys = controller.computeCPMByGranularity(start, end, interval);
//
