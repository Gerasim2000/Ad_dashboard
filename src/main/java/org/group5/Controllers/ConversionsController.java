...
public class ConversionsController extends GUIController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        lineChart.setTitle("Conversions");

        try {
            initialiseInputs(granularityDrop, datePickerFrom, datePickerTo, timeFrom, timeTo);
            loadDates();
            total.setText("Total: " + controller.countConversions());
            displayDate();
            Filter filter = new Filter(controller.getFirstDate(), controller.getLastDate());
            createGranularityChart(controller.countConversions(filter, Controller.GRAN_DAY), filter, Controller.GRAN_DAY, lineChart);
    ...
    public double setStat(Date start, Date end, int granularity) throws SQLException {
        int[] data = controller.countConversions(new Filter(start, end), granularity);
        return data[0];
    }
    ...
    public void showGraph() throws ParseException, SQLException {
        ...
        try {
            switch (granularityDrop.getValue()) {
                case "Day granularity" -> createGranularityChart(controller.countConversions(filter, Controller.GRAN_DAY), filter, Controller.GRAN_DAY, lineChart);
                case "Week granularity" -> createGranularityChart(controller.countConversions(filter, Controller.GRAN_WEEK), filter, Controller.GRAN_WEEK, lineChart);
                case "Month granularity" -> createGranularityChart(controller.countConversions(filter, Controller.GRAN_MONTH), filter, Controller.GRAN_MONTH, lineChart);
                case "Time of day" -> createPerformanceMetricChart(controller.countConversionsPM(filter, Controller.PERFORMANCE_DAY), filter, Controller.PERFORMANCE_DAY, lineChart);
                case "Day of week" -> createPerformanceMetricChart(controller.countConversionsPM(filter, Controller.PERFORMANCE_WEEK), filter, Controller.PERFORMANCE_WEEK, lineChart);
...
//
//
//    //TODO make function for count total number of conversions
//    @Override
//    public void initialize(URL location, ResourceBundle resources) {
//        //Giving list of 0
//
//        try {
//            loadDates();
//            total.setText("Total Conversions: "+controller.countConversions());
//            displayDate();
...
//        int[] days = controller.countConversionsByGranularity(start, end, interval);
...
//        int[] ys = controller.countConversionsByGranularity(start, end, interval);
//
