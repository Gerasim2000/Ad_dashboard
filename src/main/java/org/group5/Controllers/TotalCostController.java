...
public class TotalCostController extends GUIController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        try {
            ...
            loadDates();
            total.setText("Total: " + controller.computeTotalCost());
            displayDate();
            Filter filter = new Filter(controller.getFirstDate(), controller.getLastDate());
            createGranularityChart(controller.computeTotalCost(filter, Controller.GRAN_DAY), filter, Controller.GRAN_DAY, lineChart);
    ...
    public double setStat(Date start, Date end, int granularity) throws SQLException {
        float[] data = controller.computeTotalCost(new Filter(start, end), granularity);
        return data[0];
    }
    ...
    public void showGraph() throws ParseException, SQLException {
        ...
        try {
            switch (granularityDrop.getValue()) {
                case "Day granularity" -> createGranularityChart(controller.computeTotalCost(filter, Controller.GRAN_DAY), filter, Controller.GRAN_DAY, lineChart);
                case "Week granularity" -> createGranularityChart(controller.computeTotalCost(filter, Controller.GRAN_WEEK), filter, Controller.GRAN_WEEK, lineChart);
                case "Month granularity" -> createGranularityChart(controller.computeTotalCost(filter, Controller.GRAN_MONTH), filter, Controller.GRAN_MONTH, lineChart);
                case "Time of day" -> createPerformanceMetricChart(controller.computeTotalCostPM(filter, Controller.PERFORMANCE_DAY), filter, Controller.PERFORMANCE_DAY, lineChart);
                case "Day of week" -> createPerformanceMetricChart(controller.computeTotalCostPM(filter, Controller.PERFORMANCE_WEEK), filter, Controller.PERFORMANCE_WEEK, lineChart);
...
//
//
//    //TODO create functions for unique
//    @Override
//    public void initialize(URL location, ResourceBundle resources) {
//        try {
//            loadDates();
////            total.setText("Total Uniques: "+controller.countUniqueClicks());
////            displayDate();
...
////        int[] days = controller.countUniquesByGranularity(start, end, interval);
...
////    }
////
////    public void populateChart(Date start, Date end, int interval) {
////        int[] ys = controller.countUniquesByGranularity(start, end, interval);
////
