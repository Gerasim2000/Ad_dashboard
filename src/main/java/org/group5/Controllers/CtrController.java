...
public class CtrController extends GUIController implements Initializable {
    @FXML
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        try {
            ...
            createGranularityChart(controller.computeCTR(filter, Controller.GRAN_DAY), filter, Controller.GRAN_DAY, lineChart);
        } catch (SQLException throwables) {
            alertFail(Alert.AlertType.ERROR, throwables, "Chart load error", "Failed to load chart");
            throwables.printStackTrace();
        }
    }

    public double setStat(Date start, Date end, int granularity) throws SQLException {
        float[] data = controller.computeCTR(new Filter(start, end), granularity);
        return data[0];
    }
    ...
    public void showGraph() throws ParseException, SQLException {
        ...
        try {
            switch (granularityDrop.getValue()) {
                case "Day granularity" -> createGranularityChart(controller.computeCTR(filter, Controller.GRAN_DAY), filter, Controller.GRAN_DAY, lineChart);
                case "Week granularity" -> createGranularityChart(controller.computeCTR(filter, Controller.GRAN_WEEK), filter, Controller.GRAN_WEEK, lineChart);
                case "Month granularity" -> createGranularityChart(controller.computeCTR(filter, Controller.GRAN_MONTH), filter, Controller.GRAN_MONTH, lineChart);
                case "Time of day" -> createPerformanceMetricChart(controller.computeCTRPM(filter, Controller.PERFORMANCE_DAY), filter, Controller.PERFORMANCE_DAY, lineChart);
                case "Day of week" -> createPerformanceMetricChart(controller.computeCTRPM(filter, Controller.PERFORMANCE_WEEK), filter, Controller.PERFORMANCE_WEEK, lineChart);
...
//        float[] days = controller.computeCTRByGranularity(start, end, interval);
...
//
//        float[] ys = controller.computeCTRByGranularity(start, end, interval);
//
