...
public class CpaController extends GUIController implements Initializable {
    @FXML
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        try {
            ...
            createGranularityChart(controller.computeCPA(filter, Controller.GRAN_DAY), filter, Controller.GRAN_DAY, lineChart);
    ...
    public double setStat(Date start, Date end, int granularity) throws SQLException {
        float[] data = controller.computeCPA(new Filter(start, end), granularity);
        return data[0];
    }
    ...
    public void showGraph() throws ParseException, SQLException {
        ...
        try {
            switch (granularityDrop.getValue()) {
                case "Day granularity" -> createGranularityChart(controller.computeCPA(filter, Controller.GRAN_DAY), filter, Controller.GRAN_DAY, lineChart);
                case "Week granularity" -> createGranularityChart(controller.computeCPA(filter, Controller.GRAN_WEEK), filter, Controller.GRAN_WEEK, lineChart);
                case "Month granularity" -> createGranularityChart(controller.computeCPA(filter, Controller.GRAN_MONTH), filter, Controller.GRAN_MONTH, lineChart);
                case "Time of day" -> createPerformanceMetricChart(controller.computeCPAPM(filter, Controller.PERFORMANCE_DAY), filter, Controller.PERFORMANCE_DAY, lineChart);
                case "Day of week" -> createPerformanceMetricChart(controller.computeCPAPM(filter, Controller.PERFORMANCE_WEEK), filter, Controller.PERFORMANCE_WEEK, lineChart);
...
//        float[] days = controller.computeCPAByGranularity(start, end, interval);
...
//
//        float[] ys = controller.computeCPAByGranularity(start, end, interval);
//
